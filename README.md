...
RDBMS is a system that takes advantage of the relational model, whereby database objeects are stored in table form, it is the central concept behind database management systems such as MySQL. SQL itself is used to perform querying operations on these relational databases.

2. Why do tables need a `primary key`?
This is core to proucing normalized database, it offers a (hopefully unique) point of reference to each record from other tables and within that table itself. Where possible the primary key should be artificial, and unique.

3. What is the name given to a table column that references the primary key on another table.
Foreign key.

4. What do we need in order to have a _many to many_ relationship between two tables.
A third table the holds foreign keys and references the primary keys on related table(s).
